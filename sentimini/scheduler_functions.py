...
def add_new_simulated_text_no_sim(user,exp,tmp_date,exp_resp_rate):
	text_new = ActualTextSTM(user=user,response=None,feed_name=exp.feed_name,simulated=0,feed_id=exp.feed_id)
...
def add_new_simulated_text_notuser(user,exp,tmp_date,exp_resp_rate):
	text_new = ActualTextSTM_SIM(user=user,notuser=1,response=None,feed_name=exp.feed_name,simulated=1,feed_id=exp.feed_id)
...
def generate_random_prompts_to_show_no_sim(request,exp_resp_rate,week,number_of_prompts):
	...
	if ActualTextSTM.objects.all().filter(user=request.user).count()>0:
		ActualTextSTM.objects.all().filter(user=request.user).delete()
	...
	print("WE COUNT", working_experience.count())
	...
	for exp_name in working_experience:
		...
		if PossibleTextSTM.objects.all().filter(user=request.user).filter(feed_id=exp.feed_id).count() > 0:
			...
			print("NUMBER OF TEXTS IN ExP", )
			
			if week > 0:
				# print("WEEK STARTED")
				while (time_passed.days) < 8:
					text_new = add_new_simulated_text_no_sim(user=request.user,exp=exp,tmp_date=tmp_date,exp_resp_rate=exp_resp_rate)
...
def generate_random_prompts_to_show_nouser(request,exp_resp_rate,week,number_of_prompts):
	#Generate 100 prompts
	...
	working_experience = FeedSetting.objects.all().filter(feed_type="library")

	for exp_name in working_experience:
		tmp_date = datetime.now(pytz.UTC)
		exp = FeedSetting.objects.all().filter(feed_type="library").get(feed_id=exp_name.feed_id)
		...
		if PossibleTextSTM.objects.all().filter(user=request.user).filter(feed_id=exp.feed_id).count() > 0:
			...
			if week > 0:
				# print("WEEK STARTED")
				while (time_passed.days) < 8:
					text_new = add_new_simulated_text_notuser(user=request.user,exp=exp,tmp_date=tmp_date,exp_resp_rate=exp_resp_rate)
					#figure out conditions to stop
					tmp_date = text_new.time_to_send
